@php($title = 'Penginapan')
@extends('admin.layout', ['title' => $title])

@section('content')
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold tracking-tight">Penginapan</h1>
            <div class="mt-2 text-sm text-slate-500">Kelola hotel, homestay, villa, dan guest house di Cilacap.</div>
        </div>
        <a href="{{ route('admin.accommodations.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-white shadow-lg transition hover:-translate-y-0.5 hover:bg-primary/90 hover:shadow-xl">
        ...
        <form method="GET" class="grid gap-4 lg:grid-cols-4">
            <div class="lg:col-span-2">
                <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    ...
                    <input name="q" value="{{ $filters['q'] }}" placeholder="Cari penginapan..." class="w-full bg-transparent text-sm outline-none">
                </div>
            </div>
            <div>
                <select name="zone" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none">
            ...
            <div class="lg:col-span-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between pt-2">
                <div class="flex items-center gap-2 text-sm text-slate-500">
                    ...
                    Menampilkan {{ $accommodations->total() }} data
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('admin.accommodations.index') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold transition hover:border-primary hover:bg-primary/5">
            ...
            <table class="min-w-full text-left text-sm">
                <thead class="border-b border-slate-200 bg-slate-50">
                    <tr>
                        ...
                        <th class="px-5 py-4 font-bold text-slate-700">Zona</th>
                ...
                <tbody>
                    @foreach($accommodations as $a)
                        <tr class="border-b border-slate-200 last:border-b-0 transition hover:bg-slate-50">
                            <td class="px-5 py-4">
                                @php($images = is_array($a->images) ? $a->images : [])
                                @if(count($images))
                            ...
                            <td class="px-5 py-4">
                                ...
                                <div class="mt-1 text-xs text-slate-500">{{ $a->slug }}</div>
                            </td>
                            <td class="px-5 py-4 text-slate-600">{{ $categories[$a->category] ?? $a->category }}</td>
                            <td class="px-5 py-4 text-slate-600">{{ $locationZones[$a->location_zone] ?? $a->location_zone }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-sm text-slate-500">{{ $a->updated_at?->format('d/m/Y H:i') ?? '-' }}</td>
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.accommodations.edit', $a->id) }}" class="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-bold transition hover:border-primary hover:bg-primary/10 hover:text-primary">
                                        <i class="bi-pencil"></i>
                                        Edit
                                    </a>
                                    <form method="POST" action="{{ route('admin.accommodations.destroy', $a->id) }}" onsubmit="return confirm('Yakin ingin menghapus penginapan ini?')">
                                        @csrf
                                        @method('DELETE')
