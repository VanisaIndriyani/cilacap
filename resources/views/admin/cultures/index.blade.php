...
@section('content')
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold tracking-tight">Budaya</h1>
            <div class="mt-2 text-sm text-slate-500">Kelola budaya Cilacap: adat, kesenian, upacara, pakaian adat, sejarah budaya.</div>
        </div>
        <a href="{{ route('admin.cultures.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-white shadow-lg transition hover:-translate-y-0.5 hover:bg-primary/90 hover:shadow-xl">
        ...
        <form method="GET" class="grid gap-4 lg:grid-cols-4">
            <div class="lg:col-span-2">
                <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    ...
                    <input name="q" value="{{ $filters['q'] }}" placeholder="Cari budaya..." class="w-full bg-transparent text-sm outline-none">
                </div>
            </div>
            <div>
            ...
            <div class="lg:col-span-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between pt-2">
                <div class="flex items-center gap-2 text-sm text-slate-500">
                    ...
                    Menampilkan {{ $cultures->total() }} data
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('admin.cultures.index') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-semibold transition hover:border-primary hover:bg-primary/5">
            ...
            <table class="min-w-full text-left text-sm">
                <thead class="border-b border-slate-200 bg-slate-50">
                    <tr>
                        ...
                        <th class="px-5 py-4 font-bold text-slate-700">Tipe</th>
                ...
                <tbody>
                    @foreach($cultures as $c)
                        <tr class="border-b border-slate-200 last:border-b-0 transition hover:bg-slate-50">
                            <td class="px-5 py-4">
                                @php($images = is_array($c->images) ? $c->images : [])
                                @php($imagePath = $c->image ?? (count($images) ? $images[0] : null))
                                @if($imagePath)
                                    <div class="flex items-center gap-2">
                                        ...
                                        @endif
                                        @if(count($images) > 0)
                                            <span class="text-xs text-slate-500">+{{ count($images) }}</span>
                                        @endif
                                    </div>
                                @else
                            ...
                            <td class="px-5 py-4 text-slate-600">{{ $cultureTypes[$c->type] ?? $c->type }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.cultures.edit', $c->id) }}" class="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-bold transition hover:border-primary hover:bg-primary/10 hover:text-primary">
                                        <i class="bi-pencil"></i>
                                        Edit
                                    </a>
                                    <form method="POST" action="{{ route('admin.cultures.destroy', $c->id) }}" onsubmit="return confirm('Yakin ingin menghapus budaya ini?')">
                                        @csrf
                                        @method('DELETE')
