...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        WebsiteSetting::query()->firstOrCreate([], [
            'site_name' => 'Sistem Informasi Wisata Kabupaten Cilacap',
            'about' => 'Portal informasi wisata Kabupaten Cilacap: destinasi, kuliner khas, penginapan, budaya, dan rekomendasi perjalanan.',
            'footer_text' => '© '.now()->year.' Sistem Informasi Wisata Kabupaten Cilacap',
            'social_links' => [
        ...
        $destinations = [
            ...
            [
                ...
                'description' => 'Benteng Pendem Cilacap, dengan nama asli Kustbatterij op de Landtong te Tjilatjap, adalah markas pertahanan militer kolonial Hindia Belanda yang dibangun bertahap dari tahun 1861 hingga 1879 di ujung semenanjung pelabuhan Cilacap. Disebut \'Pendem\' (bahasa Jawa yang berarti terpendam atau tertimbun) karena seluruh arsitektur bangunan ini sengaja dirancang di bawah permukaan tanah dan dilapisi bukit pasir buatan. Taktik kamuflase ini diterapkan agar pangkalan militer tidak terlihat oleh kapal perang musuh di laut lepas sekaligus melindunginya dari bom artileri. Situs cagar budaya seluas 6,5 hektare ini menjadi saksi bisu transisi kekuasaan yang kejam di Indonesia, mulai dari masa keemasan kolonial Belanda, beralih ke pendudukan tentara Jepang pada Perang Dunia II, hingga digunakan sebagai basis latihan pasukan khusus TNI setelah kemerdekaan.',
            ...
            [
                ...
                'description' => 'Benteng Karangbolong merupakan situs pertahanan militer abad ke-19 peninggalan Hindia Belanda yang terletak di ujung timur Pulau Nusakambangan. Benteng ini dibangun hampir bersamaan dengan Benteng Pendem, berfungsi sebagai lapis pertama pertahanan artileri pantai (kustbatterij) untuk mengontrol dan menghancurkan kapal musuh sebelum memasuki Selat Nusakambangan menuju Pelabuhan Cilacap. Kondisi benteng ini menyatu secara dramatis dengan alam sekitarnya; akar-akar pohon beringin raksasa tua tumbuh menjalar mencengkeram dinding-dinding bata tebal benteng yang terkelupas, menciptakan atmosfer petualangan kuno ala Indiana Jones. Di dalam kompleks terdapat reruntuhan ruang meriam besar, menara pengawas pantai, barak pasukan, dan gudang penyimpanan logistik perang.',
        ...
        $brekecek = Culinary::query()->firstOrCreate(
            ['name' => 'Brekecek Pathak Jahan'],
            [
                'short_description' => 'Kuliner khas berbahan ikan dengan bumbu rempah.',
                'history' => 'Masakan tradisional khas Cilacap yang populer di berbagai acara.',
                'main_ingredients' => ['Ikan', 'Rempah', 'Cabai'],
        ...
        Culinary::query()->firstOrCreate(
            ['name' => 'Sate Martawi'],
            [
                'short_description' => 'Sate khas dengan bumbu kental dan cita rasa unik.',
                'main_ingredients' => ['Daging', 'Bumbu kacang'],
                'location_zone' => 'cilacap-kota',
                'is_popular' => true,
                'is_published' => true,
            ]
        ...
        $hotel = Accommodation::query()->firstOrCreate(
            ['name' => 'Hotel Cilacap Premium'],
            [
                'category' => 'hotel',
                'location_zone' => 'cilacap-kota',
        ...
        $culture = Culture::query()->firstOrCreate(
            ['name' => 'Kesenian Tradisional Cilacap'],
            [
                'type' => 'kesenian-daerah',
                'short_description' => 'Ragam kesenian daerah sebagai identitas budaya.',
                'description' => 'Kesenian daerah yang berkembang di masyarakat Cilacap.',
                'article' => 'Konten artikel lengkap dapat dikelola dari admin.',
                'is_featured' => true,
                'is_published' => true,
            ]
        );
        ...
        Testimonial::query()->firstOrCreate(
            ['name' => 'Budi Santoso', 'message' => 'Cilacap luar biasa, pantainya indah dan kulinernya enak.'],
            [
                'role' => 'Wisatawan',
                'rating' => 5,
                'is_published' => true,
        ...
        $package2 = TripPackage::query()->firstOrCreate(
            ['name' => 'Paket 2 Hari Cilacap Kota'],
            [
                'created_by' => $admin->id,
                'days' => 2,
                'location_zone' => 'cilacap-kota',
                'budget' => 'standar',
                'travel_types' => ['pantai', 'kuliner', 'budaya'],
                'description' => 'Contoh paket itinerary 2 hari di Cilacap Kota.',
                'is_active' => true,
            ]
        );

        // Ambil destinasi dari database yang baru dibuat
        $firstDestination = Destination::query()->first();
        $secondDestination = Destination::query()->skip(1)->first();
        
        if($firstDestination && !TripItineraryItem::query()->where(['trip_package_id' => $package2->id, 'day' => 1, 'sort_order' => 1])->exists()) {
            TripItineraryItem::query()->create([
                'trip_package_id' => $package2->id,
                'day' => 1,
                'time' => '08.00',
                'sort_order' => 1,
                'title' => null,
                ...
                'itemable_id' => $firstDestination->id,
            ]);
        }
        
        if($secondDestination && !TripItineraryItem::query()->where(['trip_package_id' => $package2->id, 'day' => 1, 'sort_order' => 2])->exists()) {
            TripItineraryItem::query()->create([
                'trip_package_id' => $package2->id,
                'day' => 1,
                'time' => '11.00',
                'sort_order' => 2,
                'itemable_type' => Destination::class,
                'itemable_id' => $secondDestination->id,
            ]);
        }
        
        if(!TripItineraryItem::query()->where(['trip_package_id' => $package2->id, 'day' => 1, 'sort_order' => 3])->exists()) {
            TripItineraryItem::query()->create([
                'trip_package_id' => $package2->id,
                'day' => 1,
                'time' => '13.00',
                'sort_order' => 3,
                'itemable_type' => Culinary::class,
                'itemable_id' => $brekecek->id,
            ]);
        }
        
        if(!TripItineraryItem::query()->where(['trip_package_id' => $package2->id, 'day' => 2, 'sort_order' => 1])->exists()) {
            TripItineraryItem::query()->create([
                'trip_package_id' => $package2->id,
                'day' => 2,
                'time' => '08.00',
                'sort_order' => 1,
                'itemable_type' => Culture::class,
                'itemable_id' => $culture->id,
            ]);
        }
        
        $thirdDestination = Destination::query()->skip(2)->first();
        if($thirdDestination && !TripItineraryItem::query()->where(['trip_package_id' => $package2->id, 'day' => 2, 'sort_order' => 2])->exists()) {
            TripItineraryItem::query()->create([
                'trip_package_id' => $package2->id,
                'day' => 2,
                'time' => '15.00',
                'sort_order' => 2,
                'itemable_type' => Destination::class,
                'itemable_id' => $thirdDestination->id,
            ]);
        }
        
        if(!TripItineraryItem::query()->where(['trip_package_id' => $package2->id, 'day' => 2, 'sort_order' => 3])->exists()) {
            TripItineraryItem::query()->create([
                'trip_package_id' => $package2->id,
                'day' => 2,
                'time' => '19.00',
                'sort_order' => 3,
                'itemable_type' => Accommodation::class,
